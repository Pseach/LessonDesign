...
int Computer_Query() {		//查询机位
	if (Temp_User.Type == 0) {//普通用户
		for (; is_run(); delay_fps(270)) {
			...
			xyprintf(Data1x, Data1y, "可用机房");
			xyprintf(Data2x, Data2y, "可用机位");
			//xyprintf(Data3x, Data3y, "状态");
			int cnt = 0;
			while (ComputerRoom_Point) {
				if (
					//ComputerRoom_Point->ComputerRoom_Data.ComputerRoom_State == 1 && 
					ComputerRoom_Point->ComputerRoom_Data.Computer_Data.Computer_State == 1) {
...
int Computer_Room_Query() {	//查询机房
	if (Temp_User.Type == 0) {//普通用户
		for (; is_run(); delay_fps(270)) {
			...
			xyprintf(Data1x, Data1y, "可用机房");
			xyprintf(Data2x, Data2y, "可用时段");//可用时段
			int cnt = 0;
			while (ComputerRoom_Point) {
				if (ComputerRoom_Point->ComputerRoom_Data.ComputerRoom_State==1) {
					Data1y += QuarryFont_Size;
					Data2y += QuarryFont_Size;
					xyprintf(Data1x, Data1y, "%s", ComputerRoom_Point->ComputerRoom_Data.ComputerRoom_Name);
